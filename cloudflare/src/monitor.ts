...
export async function runScheduled(env: ApiEnv): Promise<void> {
  ...
  for (const [index, result] of results.entries()) {
    if (result.status === "rejected") console.error("chain sync failed", [...networks.keys()][index], result.reason);
  }
  await expirePendingIntents(env.DB);
  await deliverWebhooks(env);
  await dispatchSweeps(env);
}
...
export async function deliverWebhooks(env: ApiEnv): Promise<void> {
  ...
  if (endpoint.protocol !== "https:" && endpoint.protocol !== "http:") throw new Error("PAYMENT_WEBHOOK_URL must be http(s)");
  const now = unixNow();
