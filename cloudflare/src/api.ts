...
import { all, errorText, randomId, runScheduled, unixNow } from "./monitor";
import type {
  ApiEnv,
  IntentRow,
  NetworkConfig,
...
async function createIntent(request: Request, env: ApiEnv): Promise<Response> {
  ...
  const depositAddress = deriveAddress(env.DEPOSIT_XPUB, counter.derivation_index);
  const now = unixNow();
...
export class SweepCoordinator extends WorkerEntrypoint<ApiEnv> {
  ...
  async registerSweepTransaction(jobId: string, owner: string, kind: "gas" | "sweep", rawTransaction: Hex): Promise<SweepTransaction> {
    ...
    if (transaction.chainId !== network.chainId || !transaction.to || transaction.nonce === undefined || transaction.gas === undefined) {
    ...
    if (kind === "gas") {
      ...
      if (!locked.token_address) {
        ...
        if (!isAddressEqual(transaction.to, locked.token_address) || value !== 0n || data.toLowerCase() !== canonicalData.toLowerCase()
