...
export async function allocateTurnkeyAddress(
  env: TurnkeyAddressEnv,
  index: number,
): Promise<Address> {
  validateConfig(env, env.TURNKEY_WALLET_ID);
  if (!Number.isSafeInteger(index) || index < 0 || index >= 0x80000000)
    throw new Error("deposit address space exhausted");
  ...
  if (
    intent.walletId !== env.TURNKEY_WALLET_ID ||
    intent.persist !== true ||
    !Array.isArray(accounts) ||
    accounts.length !== 1 ||
    !isObject(accounts[0]) ||
    accounts[0].curve !== "CURVE_SECP256K1" ||
    accounts[0].pathFormat !== "PATH_FORMAT_BIP32" ||
    accounts[0].path !== path ||
    accounts[0].addressFormat !== "ADDRESS_FORMAT_ETHEREUM"
  ) {
  ...
  if (
    ...
    getAddress(addresses[0]) === zeroAddress
  ) {
...
export async function signTurnkeyTransaction(
  ...
  if (!isAddress(address) || getAddress(address) === zeroAddress)
    throw new Error("Turnkey signing address is invalid");
  if (!validHex(unsignedTransaction, MAX_TRANSACTION_BYTES))
  ...
  if (
    intent.signWith !== getAddress(address) ||
    intent.unsignedTransaction !== unsignedTransaction.slice(2) ||
    intent.type !== "TRANSACTION_TYPE_ETHEREUM"
  )
    throw new Error("Turnkey activity intent mismatch");
  const signed = objectField(
    objectField(activity, "result"),
  ...
  if (!validHex(normalized, MAX_TRANSACTION_BYTES))
    throw new Error("Turnkey returned an invalid signed transaction");
  return normalized;
}
...
function completedActivity(
  ...
  if (activity.organizationId !== organizationId || activity.type !== type)
    throw new Error("Turnkey activity identity mismatch");
...
function validateConfig(env: TurnkeyEnv, walletId?: string): void {
  if (!/^[A-Za-z0-9_-]{1,200}$/.test(env.TURNKEY_ORGANIZATION_ID))
    throw new Error("TURNKEY_ORGANIZATION_ID is invalid");
  if (!/^(02|03)[0-9a-fA-F]{64}$/.test(env.TURNKEY_API_PUBLIC_KEY))
    throw new Error("TURNKEY_API_PUBLIC_KEY is invalid");
  if (!/^[0-9a-fA-F]{64}$/.test(env.TURNKEY_API_PRIVATE_KEY))
    throw new Error("TURNKEY_API_PRIVATE_KEY is invalid");
