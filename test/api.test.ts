...
describe("payment API", () => {
  ...
  it("creates, polls, and safely replays an exact payment intent", async () => {
    ...
    expect(
      (turnkeyRequests[0].body.parameters as { accounts: Array<{ path: string }> }).accounts[0]
        .path,
...
describe("sweep coordinator", () => {
  it("stores signed transactions idempotently and caps cumulative gas funding", async () => {
    ...
    await bindings.DB.batch([
      bindings.DB.prepare(`INSERT INTO payment_intents
        (id,idempotency_key,request_hash,kind,external_id,chain,chain_id,asset,token_address,decimals,expected_amount,expected_units,
         ...
         VALUES (?,?,?,'payment','order','test',1337,'USDC','0x9999999999999999999999999999999999999999',6,'0.0001','100',?,900,1,2,'paid',?,'{}',?,?)`).bind(
        intentId,
        randomId("idem"),
        "a".repeat(64),
        deposit,
    ...
    const firstRaw = await signer.signTransaction({
      type: "legacy",
      chainId: 1337,
      nonce: 0,
      to: deposit,
    ...
    const secondRaw = await signer.signTransaction({
      type: "legacy",
      chainId: 1337,
      nonce: 1,
      to: deposit,
    ...
    expect((await coordinator.registerSweepTransaction(jobId, owner, "gas", firstRaw)).id).toBe(
      first.id,
    );
    await coordinator.registerSweepTransaction(jobId, owner, "gas", secondRaw);
    const rows = await bindings.DB.prepare(
      "SELECT amount_units FROM sweep_transactions WHERE sweep_job = ? ORDER BY nonce",
    )
      .bind(jobId)
      .all<{ amount_units: string }>();
...
describe("confirmation and reorg state", () => {
  ...
  it("emits each paid transition once and reverses it after an orphaned block", async () => {
    ...
    await bindings.DB.prepare(`INSERT INTO payment_transactions
      ...
      .bind(randomId("ptx"), intentId, `0x${"1".repeat(64)}`, `0x${"2".repeat(64)}`, now, now, now)
      .run();
  ...
  it("collects a confirmed late payment without crediting it", async () => {
    ...
    await bindings.DB.batch([
      bindings.DB.prepare(`INSERT INTO payment_intents
        (id,idempotency_key,request_hash,kind,external_id,chain,chain_id,asset,token_address,decimals,expected_amount,expected_units,
         deposit_address,derivation_index,start_block,confirmations,status,expires_at,metadata,created_at,updated_at)
         VALUES (?,?,?,'payment','late-order','test',1337,'USDC','0x9999999999999999999999999999999999999999',6,'0.0001','100',?,904,1,2,'expired',?,'{}',?,?)`).bind(
        intentId,
        randomId("idem"),
        "e".repeat(64),
      ...
      bindings.DB.prepare(`INSERT INTO payment_transactions
        (id,payment_intent,chain,tx_hash,event_index,asset,from_address,to_address,amount_units,block_number,block_hash,block_timestamp,canonical,created_at,updated_at)
        VALUES (?,?,'test',?,0,'USDC','0x5555555555555555555555555555555555555555','0x8888888888888888888888888888888888888888','100',10,?,?,1,?,?)`).bind(
        randomId("ptx"),
        intentId,
        `0x${"3".repeat(64)}`,
        `0x${"4".repeat(64)}`,
  ...
  it("dispatches due sweeps even when webhook configuration is broken", async () => {
    ...
    await bindings.DB.batch([
      bindings.DB.prepare(`INSERT INTO payment_intents
        (id,idempotency_key,request_hash,kind,external_id,chain,chain_id,asset,token_address,decimals,expected_amount,expected_units,
         deposit_address,derivation_index,start_block,confirmations,status,expires_at,metadata,created_at,updated_at)
         VALUES (?,?,?,'invoice','dispatch-test','test',1337,'ETH','',18,'1','1',?,903,1,2,'paid',?,'{}',?,?)`).bind(
        intentId,
        randomId("idem"),
        "d".repeat(64),
