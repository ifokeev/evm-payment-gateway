import { decodeFunctionData, erc20Abi, keccak256, parseTransaction, recoverTransactionAddress, type Hex, type TransactionSerialized } from "viem";
import { privateKeyToAccount } from "viem/accounts";
import { afterEach, describe, expect, it, vi } from "vitest";
import sweeper from "../src/sweeper";
import type { SweepCoordinatorService, SweepJob, SweepMessage, SweepOutcome, SweepTransaction, SweeperEnv } from "../src/types";

const depositAccount = privateKeyToAccount("0x0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef");
...
describe("sweeper runtime", () => {
  ...
  it("automatically funds only the token wallet's missing gas", async () => {
    ...
    expect(fixture.state.turnkeyRequests).toBe(0);
    const transaction = parseTransaction(fixture.state.registered!);
    expect(await recoverTransactionAddress({ serializedTransaction: fixture.state.registered! as TransactionSerialized })).toBe(fixture.gasAccount.address);
  ...
  it("signs the complete token balance after gas is available", async () => {
    ...
    const transaction = parseTransaction(fixture.state.registered!);
    expect(await recoverTransactionAddress({ serializedTransaction: fixture.state.registered! as TransactionSerialized })).toBe(depositAccount.address);
...
function sweepFixture(rpcGasPrice: bigint, maxGasPriceWei: string): {
  ...
  const state: { registered?: Hex; released?: SweepOutcome; turnkeyRequests: number } = { turnkeyRequests: 0 };
  const gateway = {
    claimSweep: vi.fn(async () => job),
    registerSweepTransaction: vi.fn(async (_jobId: string, _owner: string, kind: "gas" | "sweep", raw: Hex): Promise<SweepTransaction> => {
  ...
  vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    const body = JSON.parse(await request.text()) as { id?: number; method?: string; params?: unknown[]; parameters?: Record<string, unknown> };
    if (request.url === "https://rpc.sweeper/") {
      ...
      if (body.method === "eth_sendRawTransaction") results.eth_sendRawTransaction = keccak256(body.params![0] as Hex);
    ...
    if (request.url === "https://api.turnkey.com/public/v1/submit/sign_transaction") {
      ...
      const parameters = body.parameters as { unsignedTransaction: string; signWith: string; type: string };
  ...
  const ack = vi.fn();
  const retry = vi.fn();
  const batch = { messages: [{ body: { jobId: job.id }, ack, retry }] } as unknown as MessageBatch<SweepMessage>;
...
function tokenSweepFixture(depositGas = 0n): {
  ...
  state: { registered?: Hex; kind?: "gas" | "sweep"; released?: SweepOutcome; turnkeyRequests: number };
} {
  ...
  const state: { registered?: Hex; kind?: "gas" | "sweep"; released?: SweepOutcome; turnkeyRequests: number } = { turnkeyRequests: 0 };
  const gateway = {
    claimSweep: vi.fn(async () => job),
    registerSweepTransaction: vi.fn(async (_jobId: string, _owner: string, kind: "gas" | "sweep", raw: Hex): Promise<SweepTransaction> => {
      state.registered = raw;
  ...
  vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    const body = JSON.parse(await request.text()) as {
      id: number; method: string; params: unknown[]; parameters?: { unsignedTransaction: string; signWith: string; type: string };
    };
    ...
    if (request.url !== "https://rpc.token-sweeper/") throw new Error(`unmocked request: ${request.url}`);
    let result = "0x0";
    if (body.method === "eth_chainId") result = "0x53a";
      ...
      result = (body.params[0] as string).toLowerCase() === gasAccount.address.toLowerCase()
        ? `0x${balance.toString(16)}` : `0x${depositGas.toString(16)}`;
    } else if (body.method === "eth_sendRawTransaction") result = keccak256(body.params[0] as Hex);
    return Response.json({ jsonrpc: "2.0", id: body.id, result });
  }));
  const ack = vi.fn();
  const batch = { messages: [{ body: { jobId: job.id }, ack, retry: vi.fn() }] } as unknown as MessageBatch<SweepMessage>;
