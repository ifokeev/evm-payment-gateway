...
import { deriveStatus, eligibleForSweep, formatUnits, loadNetworks, parseAmount, paymentUri, remainingGasFunding, stableStringify } from "../src/domain";
...
describe("payment domain", () => {
  ...
  it("creates wallet-compatible links", () => {
    ...
    expect(paymentUri(network, network.tokens.USDC.address, deposit, "10000000")).toContain("@8453/transfer");
  ...
  it("validates network secrets and token recovery thresholds", () => {
    ...
    const base = {
      ...
      treasuryAddress: "0x2222222222222222222222222222222222222222", confirmations: 2,
      maxGasPriceWei: "1000000000", nativeAsset: "ETH",
    };
    ...
    expect(() => loadNetworks(JSON.stringify([{ ...base, maxGasPriceWei: "0", tokens: {} }]))).toThrow("maxGasPriceWei");
    expect(loadNetworks(JSON.stringify([{ ...base, tokens: {} }]), true).size).toBe(1);
    expect(() => loadNetworks(JSON.stringify([{ ...base, rpcUrl: "http://rpc.test", tokens: {} }]))).toThrow("HTTPS");
    expect(() => loadNetworks(JSON.stringify([{ ...base, explorerUrl: "javascript:alert(1)", tokens: {} }]))).toThrow("explorer");
    expect(() => loadNetworks(JSON.stringify([{ ...base, tokens: {}, gasPrivateKey }]))).toThrow("must not contain gasPrivateKey");
    expect(() => loadNetworks(JSON.stringify([{
      ...base, nativeAsset: "USDC", tokens: { USDC: { address: "0x9999999999999999999999999999999999999999", decimals: 6 } },
    }]))).toThrow("native asset");
    expect(() => loadNetworks(JSON.stringify([{
      ...base, tokens: { USDC: { address: base.treasuryAddress, decimals: 6 } },
    }]))).toThrow("treasury");
    expect(() => loadNetworks(JSON.stringify([{
      ...base,
